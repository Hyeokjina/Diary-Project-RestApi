...
async function loadDiary() {
    try {
        ...
        if (!response.ok) {
            // ApiResponse 형식 처리
            const errorMsg = result.message || result.error || '일기를 찾을 수 없습니다.';
            alert(errorMsg);
        ...
        // ApiResponse 형식 처리
        currentDiary = result.data || result;

        // 날짜 표시
        const dateStr = currentDiary.date || currentDiary.createdAt;
...
document.getElementById('diaryEditForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // ApiResponse 형식 처리
            const errorMsg = result.message || result.error || '일기 수정에 실패했습니다.';
...
async function deleteDiary() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // ApiResponse 형식 처리
            const errorMsg = result.message || result.error || '일기 삭제에 실패했습니다.';
            alert(errorMsg);
        }
    } catch (error) {
