...
또는 IDE에서 `BoardApplication.java`를 실행합니다.

### 3. 서버 접속
- **API Server**: `http://localhost:8080`
- **H2 Console**: `http://localhost:8080/h2-console`
  - JDBC URL: `jdbc:h2:tcp://localhost/C:\workspcae\07_RestServer\db2\tdb`
  - Username: `sa`
  - Password: `1234`
...
이 프로젝트는 개발 환경에서 React와의 연동을 위해 CORS 설정이 포함되어 있습니다.

**Spring Boot 서버 설정** (이미 적용됨):
- `WebConfig.java`에서 `http://localhost:3000` 출처를 허용하도록 설정되어 있습니다.

**React 프로젝트 설정**:

React 프로젝트의 `package.json`에 다음 설정을 추가하세요:
...
// ❌ 잘못된 방법 (CORS 에러 발생 가능)
fetch("http://localhost:8080/api/diaries")
  .then(res => res.json())
  .then(data => console.log(data));
```

> ⚠️ **주의**: 이 CORS 설정은 개발 환경에서만 사용하는 임시 해결책입니다.  
...
- 이 프로젝트는 **로그인 상태 유지 기능(세션, 토큰, Spring Security)이 구현되어 있지 않습니다.**
- 백엔드는 오직 **요청을 받고, 검증하고, 데이터를 주고받는 역할**에만 집중합니다.
- H2 Database는 인메모리 방식으로 동작하므로, 서버 재시작 시 데이터가 초기화될 수 있습니다.
